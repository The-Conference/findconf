...
def make_parse_tyuiu(un_id, url, filter_date):
    ...
    try:
        ...
        try:
            ...
            # print(soup)
            main_container = soup.find('div', class_='col-lg-12 content')
            # print(main_container)
            year = 0
            for line in main_container.find('table', class_='aligncenter').find_all('tr'):
                if 'научные мероприятия' in line.find('td').text.lower():
                    year = int(line.find('td').text.split()[-2])
                    if year < filter_date.year:
                        print(f'Не найдено данных новее {filter_date}')
                        return
                if '№' in line.find('td').text:
                    continue
                ...
                if 'конфер' in line.find_all('td')[1].text.lower():
                    ...
                    dates_ = f"01 {line.find_all('td')[2].text}" if len(line.find_all('td')[2].text.split()) == 1 else \
                            f"{line.find_all('td')[2].text} {year}"

                    conf_name = normalise_str(line.find_all('td')[1].text)
                    ...
                    conf_id = f"{un_id}_{line.find_all('td')[0].text}_{conf_date_begin}_{conf_date_end}"
                    hash_ = str(hashlib.md5(bytes(conf_id, 'utf-8')).hexdigest())
                    conf_card_href = ''
                    conf_address = normalise_str(line.find_all('td')[3].text)

                    conf_s_desc = conf_name
                    conf_desc = conf_name

                    reg_date_begin = ''
                    reg_date_end = str(find_date_in_string(date_str_prep(line.find_all('td')[5].text))[0].date()) if \
                                    line.find_all('td')[5].text != '' and find_date_in_string(date_str_prep(line.find_all('td')[5].text)) else ''
                    reg_href = ''

                    org_name = ''
                    contacts = normalise_str(line.find_all('td')[4].text)
...
def parser_tyuiu(un_id, urls, date_):
    try:
        for url in urls:
            make_parse_tyuiu(un_id, url, date_)
...
if __name__ == '__main__':
    print(parser_tyuiu('tyuiu', ['https://www.tyuiu.ru/1028-2/konferentsii-2/'], datetime.strptime('2023.01.01', '%Y.%m.%d')))
