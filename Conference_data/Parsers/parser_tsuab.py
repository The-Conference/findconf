...
result = []
confer = []
async def get_tsuab_conf_data(session, url, page, year):
    params = {
        'page': f'{page}',
    ...
    try:
        async with session.get(url, params=params, headers=headers, timeout=20) as response:
            ...
            for conf in main_container.find('table').find_all('tr'):
                if conf.find('a'):
                    link = f"https://tsuab.ru{conf.find('a').get('href')}"
...
async def make_queue_tsuab(un_id, url, filter_date):
    try:
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
            ...
            try:
                async with session.get(url, headers=headers, timeout=20) as response:
                    ...
                    soup = BeautifulSoup(resp, 'lxml')
                    # print(soup)
                    main_container = soup.find('div', class_='events-list-include')

                    for conf in main_container.find_all('div', class_='events-list-item'):
                        if conf.find('a'):
                            link = f"https://tsuab.ru{conf.find('a').get('href')}"
                            title = normalise_str(conf.find('div', class_='events-list-item__title').get('title'))
                            start = normalise_str(conf.find('div', class_='events-list-item__date events-list-item__date--start').text)
                            end = normalise_str(conf.find('div', class_='events-list-item__date events-list-item__date--end').text)
                            confer.append(
                                {
                                    'title': title,
                                    'link': link,
                    ...
                    try:
                        for n, conf in enumerate(confer):
                            # print(conf)
                            if find_date_in_string(conf['start'])[0] >= filter_date or \
                                    find_date_in_string(conf['end'])[0] >= filter_date:
                                task = asyncio.create_task(parser_tsuab_pages(session, un_id, conf, n))
                                tasks.append(task)
                    except Exception as e:
...
async def parser_tsuab_pages(session, un_id, conf, n):
    try:
        async with session.get(url=conf['link'], headers=headers, timeout=20) as response:
            ...
            conf_date_begin = str(list(find_date_in_string(conf['start']))[0].date()) if \
                list(find_date_in_string(conf['start'])) else ''
            conf_date_end = str(list(find_date_in_string(conf['end']))[0].date()) if \
                list(find_date_in_string(conf['end'])) else ''
...
def parser_tsuab(un_id, url, date_):
    try:
        asyncio.run(make_queue_tsuab(un_id, url, date_))
...
if __name__ == '__main__':
    print(parser_tsuab('tsuab', 'https://tsuab.ru/events/?SECTION_ID=264', datetime.strptime('2023.01.01', '%Y.%m.%d')))
