...
async def make_queue_narfu(un_id, url, filter_date):
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.get(url=url, headers=headers, timeout=20) as response:
                    ...
                    main_container = soup.find('div', class_='work_area').find('div', class_='events')

                    for confs in main_container.find_all('div', class_='row-fluid block_events'):
                        for conf in confs.find_all('a', class_='text'):
                            title = normalise_str(conf.get_text(separator=' '))
                            # print(title)
                            if 'конфер' in title.lower():
                                conf_url = f"https://narfu.ru{conf.get('href').strip()}"
                                task = asyncio.create_task(parser_narfu_pages(session, un_id, conf_url, filter_date))
                                tasks.append(task)
                        # break
...
async def parser_narfu_pages(session, un_id, url, filter_date):
    try:
        async with session.get(url=url, headers=headers, timeout=20) as response:
            if response.status == 404:
                print(f'{response.status} - Нет такой страницы {url}')
            ...
            conf_name = normalise_str(conf_block.find('h5').text)
            ...
            lines = conf_block.find_all(['p', 'ul', 'h3'])
...
def parser_narfu(un_id, url, date_):
    try:
        asyncio.run(make_queue_narfu(un_id, url, date_))
...
if __name__ == '__main__':
    print(parser_narfu('narfu', 'https://narfu.ru/science/nauchnye-meropriyatiya/', datetime.strptime('2023.01.01', '%Y.%m.%d')))
