...
async def make_queue_petrsu(un_id, url, filter_date):
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.get(url=url, headers=headers, timeout=20) as response:
                    ...
                    # print(soup)
                    main_container = soup.find('div', class_='col-sm-8 col-xs-12 page-content')
                        ...
                        try:
                            async with session.get(url=url_, headers=headers, timeout=20) as response:
                                ...
                                conf_block = soup.find('div', class_='col-sm-8 col-xs-12 page-content'). \
                                    find('ul', class_='slist-2').find_all('li')

                                for n, conf in enumerate(conf_block):
                                    title = conf.find('div', class_='media-body media-middle').find('a').text.strip()
                                    if 'конфер' in title:
                                        conf_url = f"https://petrsu.ru{conf.find('div', class_='media-body media-middle').find('a').get('href').strip()}"
                                        # print(title, conf_url)
                                        task = asyncio.create_task(
                                            parser_petrsu_pages(session, un_id, conf_url, filter_date, n, page))
                                        tasks.append(task)
                                    # break
...
async def parser_petrsu_pages(session, un_id, url, filter_date, n, page):
    try:
        async with session.get(url=url, headers=headers, timeout=20) as response:
            if response.status == 404:
                print(f'{response.status} - Нет такой страницы {url}')
            ...
            conf_name = normalise_str(conf_block.find('h1', class_='page-content-title').text)
            ...
            for line in list(lines):
                ...
                if org_name == '' and ('организатор' in line.text.lower() or 'проводит' in line.text.lower()):
                ...
                if line.find('a') and  line.find('a').get('href') and 'mailto' in line.find('a').get('href'):
...
def make_parse_petsu(un_id, url, filter_date):
    try:
        ...
        try:
            ...
            conf_name = normalise_str(soup.find('div', id='container').find('div', id='conf_name').get_text(separator=" "))
            local = False if 'международн' in conf_name.lower() else True
            ...
            conf_date_begin = str(list(find_date_in_string(conf_name.lower()))[0].date()) if \
                list(find_date_in_string(conf_name.lower())) else ''
            conf_date_end = str(list(find_date_in_string(conf_name.lower()))[1].date()) if \
                len(list(find_date_in_string(conf_name.lower()))) > 1 else ''
            ...
            reg_href = ''
            conf_desc = ''
            org_name = 'ФГБОУ ВО «Петрозаводский государственный университет»'
            themes = ''
            online = False
            conf_href = ''
            offline = False
...
def parser_petrsu(un_id, url, date_):
    try:
        make_parse_petsu(un_id, url[0], date_)
        asyncio.run(make_queue_petrsu(un_id, url[1], date_))
...
if __name__ == '__main__':
    print(parser_petrsu('petrsu', ['https://conf.petrsu.ru/index.php',
