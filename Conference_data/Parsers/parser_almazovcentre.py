...
cookies = {
    ...
    '_ga': 'GA1.2.1607995547.1675767069',
    '_gid': 'GA1.2.1267382621.1675767069',
    'showPopup': 'true',
}
...
async def make_queue_almazovcentre(un_id, url, filter_date):
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                while True:
                    page += 1
                    url_ = f'{url}&paged={page}' if page > 1 else url
                    try:
                        async with session.get(url=url_, cookies=cookies, headers=headers, timeout=20) as response:
                            response_text = await response.text()
                            soup = BeautifulSoup(response_text, 'lxml')

                            main_container = soup.find('div', id='main-col').find('div', id='content')

                            if ('Ошибка 404' in main_container.find('article', class_='post').find('h2',
                                                                                                   class_='posttitle').text) or \
                                    page > 3:  # иначе старые все
                                print('Закончились страницы для обработки.')
                                break

                            print(f'{response.status} Обрабатываем страницу {page}, ссылка {url_}')
                            conf_block = main_container.find_all('article')

                            for n, conf in enumerate(conf_block):
                                title = normalise_str(conf.find('header').find('a').text)
                                if 'конфер' in title:
                                    conf_url = f"{conf.find('header').find('a').get('href').strip()}"
                                    # print(title, conf_url)
                                    task = asyncio.create_task(parser_almazovcentre_pages(session, un_id,
                                                                                          conf_url, url, filter_date, n,
                                                                                          page))
                                    tasks.append(task)
                            # break

                    except Exception as e:
                        raise Exception(f'Не удалось обработать ссылку {n} страницы {page} в {__name__} для {url_}\n{e}')
...
async def parser_almazovcentre_pages(session, un_id, url, source_url, filter_date, n, page):
    try:
        async with session.get(url=url, cookies=cookies, headers=headers, timeout=20) as response:
            ...
            soup = BeautifulSoup(response_text, 'lxml')
            try:
                conf_block = soup.find('div', class_='entry-container fix')
            except:
            ...
            conf_name = normalise_str(soup.find('article').find('div', class_='title').find('a').text)
            local = False if 'международн' in conf_name.lower() else True

            conf_card_href = url

            reg_href = ''
            if conf_block.find('div', class_='personal'):
                refs = conf_block.find('div', class_='personal').find_all('a')
                for ref in refs:
                    # print(ref)
                    reg_href = ref.get('href') if 'регистрация' in ref.text.lower() else 'отсутствует'

            conf_date_begin = str(list(find_date_in_string(conf_name))[0].date()) if \
                list(find_date_in_string(conf_name)) else ''
            conf_date_end = str(list(find_date_in_string(conf_name))[1].date()) if \
                len(list(find_date_in_string(conf_name))) > 1 else ''
...
def parser_almazovcentre(un_id, urls, date_):
    try:
        for url in urls:
            asyncio.run(make_queue_almazovcentre(un_id, url, date_))
...
if __name__ == '__main__':
    print(parser_almazovcentre('almazovcentre', ['http://www.almazovcentre.ru/?cat=5'],
