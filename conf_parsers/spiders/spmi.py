...
class SpmiSpider(CrawlSpider):
    ...
    rules = (
        Rule(
            LinkExtractor(restrict_css='div.konferencii a'), callback='parse_items', follow=False
        ),
    )
    ...
    def parse_items(self, response):
        ...
        if not date:
            date = header.split('\n')[-1]
        new_item.add_value('title', ''.join(header[:-1]))
        ...
        new_item.replace_value('description', descr)
