...
class VolsuSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(
            self.start_urls.pop(), callback=self.parse_links, meta={'playwright': True}
        )
    ...
    def parse_links(self, response):
        for card in response.css('a.card'):
            ...
            if 'онференц' in title.lower():
                yield scrapy.Request(
                    response.urljoin(link), callback=self.parse_items, meta={'playwright': True}
                )
        next_page = response.xpath("//a[contains(text(), 'Следующая')]/@href").get()
        yield scrapy.Request(
            response.urljoin(next_page), callback=self.parse_links, meta={'playwright': True}
        )
    ...
    def parse_items(self, response):
        ...
        for line in response.xpath(
            "//div[@class='news-detail']//*[self::p or self::div or self::ul]"
        ):
