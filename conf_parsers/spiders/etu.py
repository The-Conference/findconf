...
class EtuSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://etu.ru/ru/nauchnaya-i-innovacionnaya-deyatelnost/konferencii-seminary-vystavki/v-spbgetu'
    ]
    ...
    def parse(self, response, **kwargs):
        ...
        for row in table.xpath('.//tr'):
            ...
            for link in links:
                ...
                if (
                    not url.endswith(tuple(IGNORED_EXTENSIONS))
                    and title
                    and 'конф' in title.lower()
                ):
                    yield scrapy.Request(
                        response.urljoin(url),
                        callback=self.parse_items,
                        meta={'dates': dates, 'title': title, 'contacts': contacts},
                    )
