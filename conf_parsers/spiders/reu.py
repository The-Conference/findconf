...
class ReuSpider(CrawlSpider):
    ...
    start_urls = [
        'https://рэу.рф/science/direktsiya-po-nauke-i-innovatsiyam/orgnirupr/otdel_nauchnyh_meropriyatij/'
        'informatsiya_o_nauchnykh_meropriyatiyakh/aktualnye_nauchnye_meropriyatiya'
    ]
    rules = (
        Rule(
            LinkExtractor(restrict_css='div.event__block-text--data-link'),
    ...
    def parse_items(self, response):
        ...
        dates = (
            response.css('div.page-event-date::text').get()
            or response.xpath("string(//div[@class='mydate'])").get()
        )
        new_item = get_dates(dates, new_item)

        block = response.xpath("//div[@class='page-event-text']") or response.xpath(
