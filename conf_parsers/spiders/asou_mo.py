...
class AsouMoSpider(CrawlSpider):
    ...
    rules = (
        Rule(
            LinkExtractor(restrict_css='a.hentry', restrict_text='онференц'),
    ...
    def parse_items(self, response):
        ...
        new_item.add_xpath(
            'conf_address', "string(//div[contains(@class, 'tabs__place-info--loc')])"
        )
        new_item.add_xpath(
            'contacts', "string(//div[contains(@class, 'tabs__place-info--phone')])"
        )
