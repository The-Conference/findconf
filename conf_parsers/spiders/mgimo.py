...
class MgimoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(self.start_urls.pop(), callback=self.parse_links, headers=self.headers,
    ...
    def parse_links(self, response):
        ...
        for link in link_extractor.extract_links(response):
            yield scrapy.Request(link.url, callback=self.parse_items, headers=self.headers,
                                 meta={"playwright": True,
    ...
    def parse_items(self, response):
        ...
        for line in response.xpath("//div[@class='templ01__text-container post']/*[self::p or self::ul]"):
