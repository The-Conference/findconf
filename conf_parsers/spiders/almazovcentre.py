...
class AlmazovcentreSpider(CrawlSpider):
    ...
    rules = (
        Rule(
            LinkExtractor(restrict_css='a.entry-title', restrict_text='онференц'),
    ...
    def parse_items(self, response):
        ...
        new_item.add_xpath(
            'reg_href', "//div[@class='personal']/a[contains(.,'Регистрация')]/@href"
        )
        new_item = get_dates(conf_name, new_item)

        for line in response.xpath(
            "//div[@class='entry fix']/*[self::p or self::div[not(@class)]]"
        ):
