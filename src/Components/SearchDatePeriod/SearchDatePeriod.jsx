...
const SearchDatePeriod = () => {
  ...
  let newPeriod = periods.split(",").map((item) => new Date(item));
  let range = getDatesInRange(newPeriod[0], newPeriod[1]);
        ...
        {!isLoading &&
          ...
          conferences
            .filter(
              (el) =>
                range.includes(
                  new Date(el.conf_date_begin).toLocaleDateString()
                ) ||
                range.includes(new Date(el.conf_date_end).toLocaleDateString())
            )
            .map((el) => (
...
export default SearchDatePeriod;
